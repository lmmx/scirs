...
impl WaveletPacketTree2D {
    ...
    pub fn get_packet_mut(&mut self, level: usize, row: usize, col: usize) -> Option<&mut WaveletPacket2D> {
    ...
    pub fn reconstruct(&self) -> SignalResult<Array2<f64>> {
        ...
        if self.max_level == 0 {
            ...
            return Ok(self.get_packet(0, 0, 0)
    ...
    pub fn reconstruct_selective(&self, selected_packets: &[(usize, usize, usize)]) -> SignalResult<Array2<f64>> {
...
fn decompose_node(
    ...
    let parent = tree.get_packet(level, row, col)
        .ok_or_else(|| SignalError::ValueError(
            format!("Missing wavelet packet at level {}, position ({}, {})", level, row, col)
        ))?
        .clone();
    ...
    let child_ll = WaveletPacket2D::new(
        ...
        child_col_base, 
        ll,
        format!("{}{}{}", parent.path, if parent.path.is_empty() { "" } else { "-" }, "LL"),
    );
    ...
    let child_lh = WaveletPacket2D::new(
        ...
        child_col_base + 1, 
        lh,
        format!("{}{}{}", parent.path, if parent.path.is_empty() { "" } else { "-" }, "LH"),
    );
    ...
    let child_hl = WaveletPacket2D::new(
        ...
        child_col_base, 
        hl,
        format!("{}{}{}", parent.path, if parent.path.is_empty() { "" } else { "-" }, "HL"),
    );
    ...
    let child_hh = WaveletPacket2D::new(
        ...
        child_col_base + 1, 
        hh,
        format!("{}{}{}", parent.path, if parent.path.is_empty() { "" } else { "-" }, "HH"),
    );
    ...
    #[cfg(feature = "parallel")]
    {
        rayon::join(
            || rayon::join(
                || decompose_node(tree, child_level, child_row_base, child_col_base, max_level, mode),
                || decompose_node(tree, child_level, child_row_base, child_col_base + 1, max_level, mode),
            ),
            || rayon::join(
                || decompose_node(tree, child_level, child_row_base + 1, child_col_base, max_level, mode),
                || decompose_node(tree, child_level, child_row_base + 1, child_col_base + 1, max_level, mode),
            ),
        );
    }
    ...
    F: Fn(&WaveletPacket2D) -> bool + Copy,
{
    ...
    // Recursively decompose each child
    decompose_node_selective(tree, child_level, child_row_base, child_col_base, max_level, criterion, mode)?;
    decompose_node_selective(tree, child_level, child_row_base, child_col_base + 1, max_level, criterion, mode)?;
    decompose_node_selective(tree, child_level, child_row_base + 1, child_col_base, max_level, criterion, mode)?;
    decompose_node_selective(tree, child_level, child_row_base + 1, child_col_base + 1, max_level, criterion, mode)?;
