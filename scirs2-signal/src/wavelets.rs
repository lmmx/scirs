...
mod tests {
    ...
    fn test_complex_morlet_wavelet() {
        ...
        let wavelet_asym = complex_morlet(points, center_frequency, bandwidth, asymmetry, scale).unwrap();
        ...
        assert!(has_difference, "Asymmetric parameter should produce different wavelet values");
    }
    ...
    fn test_complex_gaussian_wavelet() {
        ...
        for i in 1..points {
            if wavelet[i].re * wavelet[i-1].re <= 0.0 && (wavelet[i].re.abs() > 1e-10 || wavelet[i-1].re.abs() > 1e-10) {
                zero_crossings += 1;
            }
        }
        
        // Higher-order Complex Gaussian should have multiple zero crossings
        assert!(zero_crossings >= 2, "Complex Gaussian should have multiple oscillations");
        ...
        assert!(zero_crossings_order1 < zero_crossings, 
    ...
    fn test_shannon_wavelet() {
        ...
        if expected_period > 0 && expected_period < points / 4 {
            ...
            let center_phases: Vec<f64> = (mid-10..mid+10)
                .map(|i| wavelet[i].arg())
                .collect();
            ...
            for i in 1..center_phases.len() {
                ...
                if diff > 0.01 { // Ignore very small changes
            ...
            if !phase_diffs.is_empty() {
                ...
                assert!((estimated_period as isize - expected_period as isize).abs() <= 5,
                       "Expected period approximately {} but got {}", expected_period, estimated_period);
            }
        }
    }
    ...
    fn test_fbsp_wavelet() {
        ...
        let calc_concentration = |w: &[Complex64]| -> f64 {
            ...
            let central_energy: f64 = w[central_start..central_end].iter().map(|c| c.norm_sqr()).sum();
        ...
        assert!(concentration_order3 > concentration_order2, 
                "Order 3 should be more concentrated than order 2");
        assert!(concentration_order4 > concentration_order3, 
        ...
        for i in 1..phases.len() {
            ...
            if diff > 0.01 && diff < PI { // Ignore very small changes and phase wrapping
                phase_diffs.push(diff);
            }
        }
    ...
    fn test_cwt_with_complex_morlet() {
        ...
        assert!(scale_energies[1] > 0.0, "CWT should capture sine wave energy");
