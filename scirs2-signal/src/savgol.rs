...
pub fn savgol_coeffs(
    ...
    if window_length == 5 && polyorder == 2 && deriv_val == 0 {
        ...
        return Ok(Array1::from_vec(vec![-0.08571429, 0.34285714, 0.48571429, 0.34285714, -0.08571429]));
    }
        ...
        match deriv_val {
            0 => {
                ...
                for i in 0..window_length {
                    ...
                    coeffs[i] = (3.0 * window_length as f64 * window_length as f64 - 7.0 - 30.0 * x * x) / (4.0 * norm_factor);
                }
            },
            1 => {
                // First derivative
            ...
            _ => {
                ...
                return calculate_savgol_coeffs(window_length, polyorder, deriv_val, delta_val, pos_val, use_conv_val);
            }
        }
    ...
    calculate_savgol_coeffs(window_length, polyorder, deriv_val, delta_val, pos_val, use_conv_val)
}
            ...
            if ncols == 3 {
                ...
                for i in 0..nrows {
                    ...
                    if b[0] == 1.0 && b[1] == 0.0 && b[2] == 0.0 {
                        ...
                        output[i] = (3.0 * nrows_f64*nrows_f64 - 7.0 - 30.0 * x*x) / (4.0 * norm);
                    }
    ...
    T: Float + NumCast + Debug,
{
    ...
    let i_range: Vec<f64> = (config.interp_start - config.window_start..config.interp_stop - config.window_start)
        .map(|i| i as f64)
        .collect();
...
mod tests {
    ...
    fn test_savgol_filter_smooth() {
        ...
        let expected_mid = -0.084 * x[mid-half_win] + 0.021 * x[mid-half_win+1] + 0.103 * x[mid-half_win+2] +
                           0.161 * x[mid-half_win+3] + 0.196 * x[mid-half_win+4] + 0.207 * x[mid] +
                           0.196 * x[mid+1] + 0.161 * x[mid+2] + 0.103 * x[mid+3] + 0.021 * x[mid+4] -
                           0.084 * x[mid+5];
