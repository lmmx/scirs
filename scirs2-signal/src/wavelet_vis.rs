...
pub fn arrange_coefficients_2d(decomposition: &Dwt2dResult) -> Array2<f64> {
    ...
    arranged.slice_mut(s![0..approx_rows, 0..approx_cols]).assign(approx);
    arranged.slice_mut(s![0..detail_v_rows, approx_cols..cols]).assign(detail_v);
    arranged.slice_mut(s![approx_rows..rows, 0..detail_h_cols]).assign(detail_h);
    arranged.slice_mut(s![approx_rows..rows, approx_cols..cols]).assign(detail_d);
...
/// assert_eq!(arranged.shape(), image.shape());
/// ```
pub fn arrange_multilevel_coefficients_2d(decompositions: &[Dwt2dResult]) -> SignalResult<Array2<f64>> {
    if decompositions.is_empty() {
        return Err(SignalError::ValueError("Decomposition list is empty".to_string()));
    }
                    ...
                    for di in 0..scale_factor {
                        for dj in 0..scale_factor {
                            if target_row + di < original_rows && target_col + dj < original_cols {
                                arranged[[target_row + di, target_col + dj]] = level.detail_h[[i, j]];
                            }
                        }
                    }
                }
            ...
            for j in 0..level_approx_cols {
                ...
                if target_row < original_rows && target_col < original_cols {
                    for di in 0..scale_factor {
                        for dj in 0..scale_factor {
                            if target_row + di < original_rows && target_col + dj < original_cols {
                                arranged[[target_row + di, target_col + dj]] = level.detail_v[[i, j]];
                            }
                        }
                    }
                }
            ...
            for j in 0..level_approx_cols {
                ...
                if target_row < original_rows && target_col < original_cols {
                    for di in 0..scale_factor {
                        for dj in 0..scale_factor {
                            if target_row + di < original_rows && target_col + dj < original_cols {
                                arranged[[target_row + di, target_col + dj]] = level.detail_d[[i, j]];
                            }
                        }
                    }
                }
...
pub fn normalize_coefficients(
    ...
    match strategy {
        NormalizationStrategy::MinMax => {
            ...
            let coeff_max = coefficients.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        ...
        NormalizationStrategy::Percentile(lower, upper) => {
            ...
            let lower_val = if lower_idx < n { values[lower_idx] } else { values[n-1] };
            let upper_val = if upper_idx < n { values[upper_idx] } else { values[n-1] };
...
pub fn count_nonzero_coefficients(
    ...
    let count_approx = decomposition.approx.iter()
        .filter(|&&x| x.abs() > threshold)
        .count();
    
    let count_h = decomposition.detail_h.iter()
        .filter(|&&x| x.abs() > threshold)
        .count();
    
    let count_v = decomposition.detail_v.iter()
        .filter(|&&x| x.abs() > threshold)
        .count();
    
    let count_d = decomposition.detail_d.iter()
        .filter(|&&x| x.abs() > threshold)
        .count();
...
pub mod colormaps {
    ...
    pub fn viridis(value: f64) -> [u8; 3] {
        ...
        let r = (70.0 * x.powi(2) - 35.0 * x - 129.0 * x.powi(3) + 133.0).max(0.0).min(255.0) as u8;
        let g = (87.0 + 192.0 * x - 386.0 * x.powi(2) + 203.0 * x.powi(3)).max(0.0).min(255.0) as u8;
        let b = (173.0 - 36.0 * x - 476.0 * x.powi(2) + 357.0 * x.powi(3)).max(0.0).min(255.0) as u8;
    ...
    pub fn plasma(value: f64) -> [u8; 3] {
        ...
        let r = (255.0 * (0.05 + 0.82 * x + 1.15 * x.powi(2) - 1.82 * x.powi(3))).max(0.0).min(255.0) as u8;
        let g = (255.0 * (-0.15 + 1.80 * x - 3.09 * x.powi(2) + 1.42 * x.powi(3))).max(0.0).min(255.0) as u8;
        let b = (255.0 * (0.51 + 0.56 * x - 2.31 * x.powi(2) + 1.24 * x.powi(3))).max(0.0).min(255.0) as u8;
...
mod tests {
    ...
    fn test_arrange_coefficients_2d() {
        ...
        for i in 0..approx_rows {
            for j in 0..approx_cols {
                ...
                assert_eq!(arranged[[approx_rows + i, approx_cols + j]], decomp.detail_d[[i, j]]);
            }
        }
    }
    ...
    fn test_normalize_coefficients() {
        ...
        // Test custom range
        let norm_range = normalize_coefficients(&coeffs, NormalizationStrategy::MinMax, Some((-1.0, 1.0)));
        assert!((norm_range[[0, 0]] - (-1.0)).abs() < 1e-10); // min should map to -1
        assert!((norm_range[[3, 3]] - 1.0).abs() < 1e-10);    // max should map to 1
        
        // Test Percentile normalization
        let norm_perc = normalize_coefficients(&coeffs, NormalizationStrategy::Percentile(25.0, 75.0), None);
        ...
        assert!(normalized_value >= 0.0 && normalized_value <= 1.0,
                "Normalized value {} should be between 0 and 1", normalized_value);
    }
