...
use crate::dwt2d::{wavedec2, waverec2, ThresholdMethod, Dwt2dResult};
    ...
    let thresholds = match method {
        DenoisingMethod::VisuShrink => {
            ...
            let sigma = (energy.detail_h + energy.detail_v + energy.detail_d).sqrt() / 
                        ((coeffs[0].detail_h.len() + coeffs[0].detail_v.len() + coeffs[0].detail_d.len()) as f64).sqrt();
...
fn estimate_noise_variance(coeffs: &Array2<f64>) -> f64 {
    ...
    if mad > 1e-10 {
        ...
        let variance = abs_coeffs.iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f64>() / abs_coeffs.len() as f64;
        variance
    }
}
...
mod tests {
    ...
    fn test_denoise_image() {
        ...
        let denoised_hard = denoise_image(&image, Wavelet::Haar, 2, 1.0, 
                                       DenoisingMethod::Hard, Some(true)).unwrap();
        
        let denoised_soft = denoise_image(&image, Wavelet::Haar, 2, 1.0,
                                       DenoisingMethod::Soft, Some(true)).unwrap();
        
        let denoised_visu = denoise_image(&image, Wavelet::Haar, 2, 0.0,
                                      DenoisingMethod::VisuShrink, Some(true)).unwrap();
