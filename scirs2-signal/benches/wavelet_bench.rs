...
use scirs2_signal::swt::{swt, swt_decompose, swt_reconstruct, iswt};
        ...
        for (wavelet, name) in &wavelets {
            for &level in &levels {
                ...
                group.bench_with_input(
                    ...
                    |b, _| {
                        b.iter(|| {
                            black_box(wavedec(&signal, *wavelet, Some(level), None).unwrap())
                        })
                    },
                );
                ...
                group.bench_with_input(
                    ...
                    |b, _| {
                        b.iter(|| {
                            black_box(waverec(&coeffs, *wavelet).unwrap())
                        })
                    },
                );
            }
        ...
        for (wavelet, name) in &wavelets {
            for &level in &levels {
                ...
                group.bench_with_input(
                    ...
                    |b, _| {
                        b.iter(|| {
                            black_box(swt_decompose(&signal, *wavelet, level, None).unwrap())
                        })
                    },
                );
                ...
                if level > 1 {
                    ...
                    group.bench_with_input(
                        ...
                        |b, _| {
                            b.iter(|| {
                                black_box(swt(&signal, *wavelet, level, None).unwrap())
                            })
                        },
                    );
                    ...
                    group.bench_with_input(
                        ...
                        |b, _| {
                            b.iter(|| {
                                black_box(iswt(&details, &approx, *wavelet).unwrap())
                            })
                        },
                    );
                }
        ...
        for (wavelet, name) in &wavelets {
            for &level in &levels {
                ...
                group.bench_with_input(
                    ...
                    |b, _| {
                        b.iter(|| {
                            black_box(wp_decompose(&signal, *wavelet, level, None).unwrap())
                        })
                    },
                );
                ...
                group.bench_with_input(
                    ...
                    |b, _| {
                        b.iter(|| {
                            black_box(reconstruct_from_nodes(&tree, &nodes).unwrap())
                        })
                    },
                );
            }
...
criterion_group!(
    wavelet_benches,
    bench_wavelets_single_level
    // Limiting to just single level for faster execution
