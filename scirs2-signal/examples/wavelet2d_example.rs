...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nMaximum multi-level reconstruction error: {:.2e}", max_multi_error);
    ...
    println!("\nOriginal coefficients: {} non-zero values", original_nonzero);
    println!("After thresholding: {} non-zero values", thresholded_nonzero);
    println!("Compression ratio: {:.2}x", original_nonzero as f64 / thresholded_nonzero as f64);
