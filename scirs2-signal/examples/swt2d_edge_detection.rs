...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Created test image ({}x{}) with circle and square", size, size);
    ...
    println!("Percentage of pixels detected as edges: {:.2}%", ratio * 100.0);
    ...
    T: num_traits::Float + num_traits::NumCast + std::fmt::Debug
{
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            edge_map[[i, j]] = (decomp.detail_h[[i, j]].powi(2) + 
                                 decomp.detail_v[[i, j]].powi(2)).sqrt();
        }
    }
