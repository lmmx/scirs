...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let size = 64;
    println!("\nCreating a {}x{} test image (checkerboard pattern)...", size, size);
    ...
    println!("\nEnergy distribution:");
    println!("  Approximation (LL): {:.2}% ({:.2})", 
             energy.approximation_percent, energy.approximation);
    println!("  Horizontal Detail (LH): {:.2}% ({:.2})", 
             100.0 * energy.horizontal.unwrap() / energy.total, energy.horizontal.unwrap());
    println!("  Vertical Detail (HL): {:.2}% ({:.2})", 
             100.0 * energy.vertical.unwrap() / energy.total, energy.vertical.unwrap());
    println!("  Diagonal Detail (HH): {:.2}% ({:.2})", 
             100.0 * energy.diagonal.unwrap() / energy.total, energy.diagonal.unwrap());
    println!("  Total Detail: {:.2}% ({:.2})", 
    ...
    println!("  Approximation (LL): {} coefficients", counts.approximation);
    println!("  Horizontal Detail (LH): {} coefficients", counts.horizontal);
    println!("  Vertical Detail (HL): {} coefficients", counts.vertical);
    println!("  Diagonal Detail (HH): {} coefficients", counts.diagonal);
    println!("  Total non-zero: {} out of {} ({:.2}%)", 
             counts.total, size * size, counts.percent_nonzero);
    ...
    let _normalized_approx = normalize_coefficients(&dwt_decomp.approx, NormalizationStrategy::MinMax, None);
    let _normalized_detail_h = normalize_coefficients(&dwt_decomp.detail_h, NormalizationStrategy::MinMax, None);
    
    println!("Normalized approximation coefficients range: [{:.2}, {:.2}]",
             _normalized_approx.iter().fold(f64::INFINITY, |a, &b| a.min(b)),
             _normalized_approx.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
    
    // Apply different normalization strategies
    println!("\nApplying different normalization strategies to horizontal detail coefficients...");
    let norm_minmax = normalize_coefficients(&dwt_decomp.detail_h, NormalizationStrategy::MinMax, None);
    let norm_abs = normalize_coefficients(&dwt_decomp.detail_h, NormalizationStrategy::Absolute, None);
    let norm_log = normalize_coefficients(&dwt_decomp.detail_h, NormalizationStrategy::Log, None);
    let norm_perc = normalize_coefficients(&dwt_decomp.detail_h, 
                                          NormalizationStrategy::Percentile(5.0, 95.0), None);
    ...
    let _heatmap_viridis = create_coefficient_heatmap(&dwt_decomp.detail_h, 
                                                   colormaps::viridis, 
                                                   Some(NormalizationStrategy::MinMax));
    let _heatmap_plasma = create_coefficient_heatmap(&dwt_decomp.detail_h, 
                                                  colormaps::plasma, 
                                                  Some(NormalizationStrategy::MinMax));
    let _heatmap_diverging = create_coefficient_heatmap(&dwt_decomp.detail_h, 
                                                     colormaps::diverging_rb, 
                                                     Some(NormalizationStrategy::Absolute));
                                                     
    println!("Generated heatmaps with shapes: viridis={:?}, plasma={:?}, diverging={:?}",
             _heatmap_viridis.shape(), _heatmap_plasma.shape(), _heatmap_diverging.shape());
