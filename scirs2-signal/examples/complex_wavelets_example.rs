...
use scirs2_signal::{
    wavelets::{cwt, complex_morlet, complex_gaussian, fbsp, shannon},
    waveforms::chirp,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Scale range: {:.1} to {:.1}\n", scales[0], scales[scales.len() - 1]);
    ...
    let theo_freq: Vec<f64> = t.iter().map(|&t_val| 5.0 + (100.0 - 5.0) * t_val / duration).collect();
    ...
    let calculate_mae = |freq: &[f64], theo: &[f64]| -> f64 {
        ...
        freq.iter().skip(skip).take(freq.len() - 2 * skip)
            .zip(theo.iter().skip(skip).take(theo.len() - 2 * skip))
            .map(|(&f, &t)| (f - t).abs())
            .sum::<f64>() / (freq.len() - 2 * skip) as f64
    };
