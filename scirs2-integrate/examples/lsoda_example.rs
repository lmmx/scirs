...
fn main() {
    ...
    println!("  Steps: {}, Function evaluations: {}", result.n_steps, result.n_eval);
    println!("  Final value: {:.6}, Exact: {:.6}", 
             result.y.last().unwrap()[0], (-5.0f64).exp());
    println!("  Error: {:.2e}", (result.y.last().unwrap()[0] - (-5.0f64).exp()).abs());
