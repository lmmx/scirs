...
fn main() {
    ...
    for &t in &exact_points {
        ...
        let idx = result.t.iter().position(|&rt| (rt - t as f64).abs() < 1e-10_f64)
            .unwrap_or_else(|| {
                result.t.iter()
                    .enumerate()
                    .min_by(|(_, a), (_, b)| {
    ...
    println!("  Number of steps: {}", result.n_steps);
    println!("  Number of function evaluations: {}", result.n_eval);
    println!("  Steps per period: {}", (result.n_steps as f64 / periods as f64));
}