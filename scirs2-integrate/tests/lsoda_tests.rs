...
fn test_lsoda_basic() {
    ...
    assert!(result.is_ok(), "Basic LSODA test failed: {:?}", result.err());
    ...
    println!("Computed: {}, Exact: {}, Relative error: {}", computed, exact, relative_error);
...
fn test_lsoda_with_stiffness_change() {
    ...
    if !result.success {
        println!("Warning: integration did not complete successfully, took {} steps", result.n_steps);
    }
    
    // Expect method switching for this problem (may not be exact count due to implementation changes)
    if let Some(msg) = result.message {
        assert!(msg.contains("Method switches"), "Expected method switching in result message");
    }
}
...
fn test_lsoda_method_switching() {
    ...
    // The test should now pass with our improved implementation
    assert!(result.is_ok(), "LSODA varying stiffness test failed: {:?}", result.err());
    ...
    if let Some(msg) = result.message {
        ...
        if let Some(start_idx) = msg.find("Method switches: ") {
            if let Some(mid_idx) = msg[start_idx..].find(" (non-stiff to stiff), ") {
                let num_switch_str = &msg[start_idx + "Method switches: ".len()..start_idx + mid_idx];
                if let Ok(num_switches) = num_switch_str.parse::<usize>() {
                    assert!(num_switches > 0, "Expected at least one method switch but found {}", num_switches);
    ...
    assert!(final_value.abs() < 1e-2, "Final value should be close to zero, got {}", final_value);
}