...
use scirs2_core::array_protocol::{
    self,
    NdarrayWrapper, GPUNdarray, GPUConfig, GPUBackend,
    DistributedNdarray, DistributedConfig, DistributionStrategy, DistributedBackend,
    matmul, add, reshape, transpose, sum,
};
...
fn main() {
    ...
    match reshape(&wrapped_a, &[9]) {
        Ok(result) => {
            ...
            if let Some(result_array) = result.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix1>>() {
    ...
    println!("Created GPU arrays with shape {:?} and {:?}", gpu_a.shape(), gpu_b.shape());
    ...
    println!("Created distributed arrays with {} and {} chunks", 
             dist_a.num_chunks(), dist_b.num_chunks());
    
    // Matrix multiplication with distributed arrays
    match matmul(&dist_a, &dist_b) {
        Ok(result) => {
            if let Some(dist_result) = result.as_any().downcast_ref::<DistributedNdarray<f64, Ix2>>() {
