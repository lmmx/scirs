...
use scirs2_core::array_protocol::{
    self, ml_ops::ActivationFunc,
    neural::{Linear, Conv2D, MaxPool2D, Sequential},
    grad::Adam,
...
fn main() {
    ...
    println!("Created dataset with {} training samples and {} validation samples",
             Dataset::len(&train_dataset), Dataset::len(&val_dataset));
    ...
    println!("Created distributed training config with {} workers using {} strategy",
    ...
    println!("Created distributed datasets with {} training samples and {} validation samples",
             dist_train_dataset.len(), dist_val_dataset.len());
    ...
    let _val_loader = DataLoader::new(
        dist_val_dataset,
        batch_size,
        false,
        None,
    ...
    match loaded_model {
        Ok((model, optimizer)) => {
            ...
            println!("Loaded optimizer: {}", if optimizer.is_some() { "yes" } else { "no" });
        },
...
fn create_dataset() -> (InMemoryDataset, InMemoryDataset) {
    ...
    let val_inputs = inputs.slice(ndarray::s![train_size..num_samples, ..]).to_owned();
    let val_targets = targets.slice(ndarray::s![train_size..num_samples, ..]).to_owned();
