...
use scirs2_core::array_protocol::{
    self, NdarrayWrapper, GPUBackend,
    ml_ops::ActivationFunc,
    auto_device::{AutoDeviceConfig, set_auto_device_config},
    neural::{Linear, Conv2D, MaxPool2D, BatchNorm, Dropout, Sequential},
    grad::{Adam},
    training::{InMemoryDataset, DataLoader, CrossEntropyLoss, Trainer},
    distributed_training::{
        DistributedTrainingConfig, DistributedStrategy,
        DistributedTrainingFactory,
    },
    serialization::{ModelSerializer, save_checkpoint, load_checkpoint, OnnxExporter},
};
...
fn main() {
    ...
    println!("Configured auto device selection with GPU threshold: {} elements",
             gpu_threshold);
    println!("Distributed threshold: {} elements",
    ...
    println!("Created dataset with {} samples, {} features, and {} classes", 
    ...
    println!("  - Gradient accumulation steps: {}", dist_config.gradient_accumulation_steps);
    ...
    println!("Split dataset into {} training samples and {} validation samples",
             train_size, num_samples - train_size);
    
    // Create distributed datasets
    let dist_train_dataset = DistributedTrainingFactory::create_dataset(
        Box::new(train_dataset),
        &dist_config,
    );
    
    let dist_val_dataset = DistributedTrainingFactory::create_dataset(
        Box::new(val_dataset),
        &dist_config,
    );
    
    println!("Created distributed datasets with {} shards each", dist_config.num_workers);
    ...
    let _dist_trainer = DistributedTrainingFactory::create_trainer(
        trainer,
        dist_config.clone(),
    );

    println!("Created distributed trainer with {} workers", dist_config.num_workers);
    ...
    println!("Note: Callbacks would typically be added to the underlying trainer before distribution");
    ...
    let result = save_checkpoint(&model, optimizer.as_ref(), &checkpoint_path, 10, metrics.clone());
    ...
    match result {
        Ok((loaded_model, loaded_optimizer, epoch, loaded_metrics)) => {
            ...
            let _resume_dist_trainer = DistributedTrainingFactory::create_trainer(
                resume_trainer,
                dist_config.clone(),
            );
    ...
    for epoch in 0..num_epochs {
        ...
        for batch in 0..num_batches {
            if (batch + 1) % (num_batches / 10).max(1) == 0 {
                let simulated_loss = 1.0 - (epoch as f64 * 0.1 + batch as f64 * 0.01 / num_batches as f64);
                print!("\rBatch {}/{} - loss: {:.4}", batch + 1, num_batches, simulated_loss);
            }
        }
        println!();
        ...
        println!("train: loss = {:.4}, accuracy = {:.4}", train_loss, train_acc);
        ...
        let _ = save_checkpoint(&model, optimizer.as_ref(), &checkpoint_path, epoch + 1, metrics);
