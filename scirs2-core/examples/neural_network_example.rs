...
use scirs2_core::array_protocol::{
    ...
    neural::{Linear, Conv2D, MaxPool2D, Dropout, Sequential, create_simple_cnn, Layer},
};
...
fn main() {
    ...
    match conv.forward(&input_wrapped) {
        Ok(output) => {
            if let Some(output_array) = output.as_any().downcast_ref::<NdarrayWrapper<f64, Ix4>>() {
                println!("Output shape from conv layer: {:?}", output_array.as_array().shape());
            } else {
                println!("Output is not of expected type");
            }
        },
    ...
    println!("Created a sequential model with {} layers", model.layers().len());
    ...
    match model.forward(&input_wrapped) {
        Ok(output) => {
            if let Some(output_array) = output.as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
                println!("Output shape from model: {:?}", output_array.as_array().shape());
    ...
    println!("Model is in training mode: {}", model.layers()[0].is_training());
