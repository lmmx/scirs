...
use scirs2_core::array_protocol::{
    self, NdarrayWrapper, ml_ops::ActivationFunc,
    neural::{Linear, Dropout, Sequential},
    grad::{Variable, Adam, Optimizer},
    training::{InMemoryDataset, DataLoader, CrossEntropyLoss, Trainer, ProgressCallback},
};
...
fn main() {
    ...
    println!("Split into {} training samples and {} validation samples",
    ...
    for (i, param) in params.iter().enumerate() {
        let param_array = if let Some(array) = param.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>() {
            array.as_array().clone()
        } else {
            println!("Parameter {} is not a 2D ndarray, using a default 2x2 array instead", i);
    ...
    let sample_input = Array2::<f64>::from_shape_fn((1, num_features), |_| {
        rand::random::<f64>() * 2.0 - 1.0
    });
    ...
    let output_array = match output.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>() {
        Some(array) => array,
        None => {
    ...
    // Get predicted class
    let predicted_class = match probs.iter()
        .enumerate()
...
fn create_simple_model() -> Sequential {
    ...
    model.add_layer(Box::new(Linear::with_shape(
        "fc_out",
        16,
        3,  // num_classes from the example
        true,
        None,
    )));
