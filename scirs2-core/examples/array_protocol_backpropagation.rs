...
use scirs2_core::array_protocol::{
    ...
    grad::{GradientTensor, Variable, Adam, Optimizer},
    training::{InMemoryDataset, DataLoader, MSELoss, Loss},
};
...
fn main() {
    ...
    for i in 0..inputs.shape()[0] {
        ...
        let z1 = match x_tensor.matmul(&w1) {
            ...
            Err(e) => {
                println!("Error in test forward pass layer 1 matrix multiplication: {}", e);
                continue; // Skip this test sample
            }
        };
        ...
        let a1 = match z1.sigmoid() {
            Ok(result) => result,
            Err(e) => {
                println!("Error in test forward pass layer 1 sigmoid activation: {}", e);
                continue; // Skip this test sample
            }
        };
        ...
        let z2 = match a1.matmul(&w2) {
            ...
            Err(e) => {
                println!("Error in test forward pass layer 2 matrix multiplication: {}", e);
                continue; // Skip this test sample
            }
        };
        ...
        let a2 = match z2.sigmoid() {
            Ok(result) => result,
            Err(e) => {
                println!("Error in test forward pass layer 2 sigmoid activation: {}", e);
                continue; // Skip this test sample
            }
        };
        ...
        println!("Input: [{:.1}, {:.1}], Target: {:.1}, Prediction: {:.4}", 
    ...
    let _data_loader = DataLoader::new(
        Box::new(dataset),
        batch_size,
        true,
        Some(42),
    ...
    println!("Created dataset and data loader with batch size {}", batch_size);
    ...
    println!("Created sequential model with {} layers", model.layers().len());
    ...
    println!("Created Adam optimizer with {} variables", optimizer.variables().len());
    ...
    for epoch in 0..num_epochs {
        ...
        let loader = DataLoader::new(
            Box::new(InMemoryDataset::from_arrays(inputs.clone(), targets.clone())),
            batch_size,
            true,
            Some(42),
        );
            ...
            for (input, target) in inputs.iter().zip(targets.iter()) {
                ...
                if let Some(loss_array) = loss.as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
                    ...
                    for var in optimizer.variables() {
                        ...
                        if let Some(array) = tensor_value.as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
    ...
    for i in 0..inputs.shape()[0] {
        ...
        // Get prediction
        let prediction = if let Some(output_array) = output.as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
...
impl GradientTensorExt for GradientTensor {
    fn to_scalar(&self) -> Option<f64> {
        ...
        if let Some(array) = self.value().as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
        ...
        if let Some(array) = self.value().as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
            ...
            if let Some(grad) = self.grad() {
                if let Some(grad_array) = grad.as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>() {
                    ...
                    println!("Would update tensor data with gradient using learning rate {}", learning_rate);
                }
            }
        }
    }
    ...
    fn matmul(&self, other: &GradientTensor) -> Result<GradientTensor, &'static str> {
        ...
        if let (Some(self_array), Some(other_array)) = (
            self.value().as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>(),
            other.value().as_any().downcast_ref::<NdarrayWrapper<f64, Ix2>>()
        ) {
            ...
            if self_dim.len() >= 1 && other_dim.len() >= 1 && self_dim[self_dim.len() - 1] == other_dim[0] {
                ...
                println!("Would perform matmul between arrays of dimensions {:?} and {:?}", self_dim, other_dim);
