...
impl JITManager {
    ...
    pub fn compile(&self, expression: &str, array_type_id: TypeId) -> CoreResult<Box<dyn JITFunction>> {
        ...
        if let Some(factory) = self.get_factory_for_array_type(array_type_id) {
            ...
            Err(CoreError::JITError(ErrorContext::new(
                format!("No JIT factory supports array type: {:?}", array_type_id)
            )))
        }
    }
    ...
    A: ArrayProtocol + Clone + Send + Sync + 'static,
{
    ...
    fn supports_jit(&self) -> bool {
        ...
        jit_manager.get_factory_for_array_type(TypeId::of::<A>()).is_some()
    }
...
mod tests {
    ...
    fn test_jit_manager() {
        ...
        let array_type_id = TypeId::of::<NdarrayWrapper<f64, ndarray::Ix2>>();
        assert!(jit_manager.get_factory_for_array_type(array_type_id).is_some());
