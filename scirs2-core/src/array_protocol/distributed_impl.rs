...
use crate::array_protocol::{ArrayProtocol, DistributedArray, NotImplemented, ArrayFunction};
...
where
    T: Clone + Send + Sync + 'static + num_traits::Zero + std::ops::Div<f64, Output=T> + Default + num_traits::One,
    D: Dimension + Clone + Send + Sync + 'static + ndarray::RemoveAxis,
{
    fn distribution_info(&self) -> HashMap<String, String> {
        ...
        info.insert("strategy".to_string(), format!("{:?}", self.config.strategy));
...
mod tests {
    ...
    fn test_distributed_ndarray_creation() {
        ...
        let total_elements: usize = dist_array.chunks().iter()
    ...
    fn test_distributed_ndarray_map_reduce() {
        ...
        let sum = dist_array.map_reduce(
            |chunk| chunk.data.sum(),
            |a, b| a + b
        );
