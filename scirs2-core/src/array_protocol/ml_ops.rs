...
use crate::array_protocol::{
    ArrayProtocol, NdarrayWrapper, array_function_dispatch,
...
fn apply_activation(x: &ndarray::ArrayBase<ndarray::ViewRepr<&f64>, IxDyn>, func: ActivationFunc) -> Array<f64, IxDyn> {
    match func {
...
array_function_dispatch!(
    fn activation(x: &dyn ArrayProtocol, func: ActivationFunc) -> Result<Box<dyn ArrayProtocol>, OperationError> {
        ...
        let result = array_ref.array_function(
            &crate::array_protocol::ArrayFunction::new("scirs2::array_protocol::ml_ops::activation"),
            ...
            if let (Some(input_array), Some(filters_array)) = (
                ...
                if input_channels != filter_in_channels {
                    return Err(OperationError::ShapeMismatch(
                        format!("Input channels ({}) doesn't match filter input channels ({})",
                ...
                // Create output array
                let output: Array<f64, Ix4> = Array::zeros((batch_size, out_height, out_width, filter_out_channels));
    ...
    fn max_pool2d(
        ...
        if implementing_args.is_empty() {
            ...
            if let Some(input_array) = input.as_any().downcast_ref::<NdarrayWrapper<f64, Ix4>>() {
                ...
                // Create output array
                let output: Array<f64, Ix4> = Array::zeros((batch_size, out_height, out_width, channels));
        ...
        // Delegate to the implementation
        let mut kwargs = HashMap::new();
        kwargs.insert("kernel_size".to_string(), Box::new(kernel_size) as Box<dyn Any>);
        kwargs.insert("stride".to_string(), Box::new(stride) as Box<dyn Any>);
        kwargs.insert("padding".to_string(), Box::new(padding) as Box<dyn Any>);
        ...
        let result = array_ref.array_function(
            &crate::array_protocol::ArrayFunction::new("scirs2::array_protocol::ml_ops::max_pool2d"),
            &[TypeId::of::<Box<dyn ArrayProtocol>>()],
    ...
    fn batch_norm(
        ...
        if implementing_args.is_empty() {
            ...
            if let (
                ...
                variance.as_any().downcast_ref::<NdarrayWrapper<f64, IxDyn>>(),
            ) {
                let input = input_array.as_array();
                ...
                // Check dimensions
                if scale.shape()[0] != channels || offset.shape()[0] != channels
                    || mean.shape()[0] != channels || variance.shape()[0] != channels {
                    return Err(OperationError::ShapeMismatch(
                        "Scale, offset, mean, and variance must match the number of channels".to_string()
                    ));
                }
        ...
        let result = array_ref.array_function(
            &crate::array_protocol::ArrayFunction::new("scirs2::array_protocol::ml_ops::batch_norm"),
            &[TypeId::of::<Box<dyn ArrayProtocol>>()],
            &[
                Box::new(input.box_clone()),
                Box::new(scale.box_clone()),
...
array_function_dispatch!(
    fn cross_entropy(
        ...
        // Get implementing args
        let boxed_args: Vec<Box<dyn Any>> = vec![
            Box::new(logits.box_clone()),
            Box::new(labels.box_clone()),
        ];
        let implementing_args = get_implementing_args(&boxed_args);
        if implementing_args.is_empty() {
            // Fallback implementation for ndarray types
            if let (Some(logits_array), Some(labels_array)) = (
                ...
                if logits.shape() != labels.shape() {
                    return Err(OperationError::ShapeMismatch(
                        format!("Logits shape {:?} doesn't match labels shape {:?}",
                ...
                let loss = match reduction {
                    ...
                    _ => return Err(OperationError::ShapeMismatch(
                        format!("Unknown reduction method: {}", reduction)
                    )),
                };
        ...
        // Delegate to the implementation
        let mut kwargs = HashMap::new();
        kwargs.insert("reduction".to_string(), Box::new(reduction.to_string()) as Box<dyn Any>);
        
        let array_ref = implementing_args[0].1;
        
        let result = array_ref.array_function(
            &crate::array_protocol::ArrayFunction::new("scirs2::array_protocol::ml_ops::cross_entropy"),
    ...
    fn dropout(
        ...
        if implementing_args.is_empty() {
            ...
            if let Some(input_array) = input.as_any().downcast_ref::<NdarrayWrapper<f64, IxDyn>>() {
                ...
                let mask = Array::from_shape_fn(
                    input.raw_dim(),
                    |_| if rng.random::<f64>() >= rate { 1.0 } else { 0.0 }
                );
    ...
    fn self_attention(
        ...
        if implementing_args.is_empty() {
            ...
            if let (
                ...
                // Check dimensions
                if k.shape()[0] != batch_size || k.shape()[2] != num_heads || k.shape()[3] != d_k
                    || v.shape()[0] != batch_size || v.shape()[1] != k_len || v.shape()[2] != num_heads {
                    return Err(OperationError::ShapeMismatch(
        ...
        let result = array_ref.array_function(
            &crate::array_protocol::ArrayFunction::new("scirs2::array_protocol::ml_ops::self_attention"),
            &[TypeId::of::<Box<dyn ArrayProtocol>>()],
            &[
                Box::new(queries.box_clone()),
                Box::new(keys.box_clone()),
