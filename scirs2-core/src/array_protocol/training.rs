...
impl InMemoryDataset {
        ...
        D2: Dimension + Send + Sync,
    {
        ...
        assert_eq!(num_samples, output_shape[0], "Inputs and targets must have the same number of samples");
        ...
        for i in 0..num_samples {
            ...
            input_samples.push(Box::new(NdarrayWrapper::new(input_array)) as Box<dyn ArrayProtocol>);

            let target_view = to_dyn_targets.index_axis(ndarray::Axis(0), i);
            let target_array = target_view.to_owned();
            target_samples.push(Box::new(NdarrayWrapper::new(target_array)) as Box<dyn ArrayProtocol>);
        }
...
impl DataLoader {
    ...
    pub fn next_batch(&mut self) -> Option<(Vec<Box<dyn ArrayProtocol>>, Vec<Box<dyn ArrayProtocol>>)> {
...
impl Loss for CrossEntropyLoss {
    fn forward(
        ...
        if let (Some(preds_array), Some(targets_array)) = (
            softmax_preds.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::IxDyn>>(),
            targets.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::IxDyn>>(),
        ) {
...
impl fmt::Display for Metrics {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}: loss = {:.4}", self.name, self.mean_loss().unwrap_or(0.0))?;
...
impl Trainer {
    ...
    fn train_batch(
        ...
        for (input, target) in inputs.iter().zip(targets.iter()) {
            ...
            // Get loss value
            if let Some(loss_array) = loss.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::IxDyn>>() {
        ...
        for _ in 0..num_batches {
            ...
            for (input, target) in inputs.iter().zip(targets.iter()) {
                ...
                if let (Some(output_array), Some(target_array)) = (
                    output.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>(),
                    target.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>(),
                ) {
                    ...
                    if output_vec.ndim() == 2 && target_vec.ndim() == 2 {
                        for (out_row, target_row) in output_vec.outer_iter().zip(target_vec.outer_iter()) {
...
fn subtract(a: &dyn ArrayProtocol, b: &dyn ArrayProtocol) -> CoreResult<Box<dyn ArrayProtocol>> {
    ...
    if let (Some(a_array), Some(b_array)) = (
        a.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>(),
        b.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix2>>(),
    ) {
...
mod tests {
    ...
    fn test_mse_loss() {
        ...
        match mse.forward(&predictions_wrapped, &targets_wrapped) {
            Ok(loss) => {
                if let Some(loss_array) = loss.as_any().downcast_ref::<NdarrayWrapper<f64, ndarray::Ix0>>() {
