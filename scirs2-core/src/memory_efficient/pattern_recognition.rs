...
impl PatternRecognizer {
    ...
    fn detect_patterns(&mut self) {
        ...
        self.patterns.retain(|pattern| pattern.is_valid(self.config.pattern_expiry));
    ...
    fn detect_matrix_patterns(&mut self, dimensions: &[usize]) {
        ...
        for i in 1..indices.len() {
            ...
            if prev_row != curr_row {
                ...
                if (prev_direction > 0 && curr_direction < 0) || (prev_direction < 0 && curr_direction > 0) {
            ...
            if count >= indices.len() / 3 && stride > 1 {
                ...
                for &block_size in &possible_block_sizes {
                    if stride < block_size && block_size % stride == 0 {
                        let pattern = ComplexPattern::BlockStrided {
                            block_size,
                            stride,
                        };
                        ...
                        if let Some(existing) = self.find_pattern(&pattern) {
                            ...
                            let pattern = RecognizedPattern::new(pattern, Confidence::Low)
                                ...
                                .with_metadata("total_transitions", &(indices.len() - 1).to_string());
                            self.patterns.push(pattern);
                        }
    ...
    fn detect_stencil_patterns(&mut self, dimensions: &[usize]) {
        ...
        for window_start in 0..indices.len().saturating_sub(5) {
            ...
            if neighbors.len() >= 3 {
                ...
                access_groups.entry(key).or_insert_with(Vec::new).push(neighbors);
            }
        }
        ...
        match pattern {
            ...
            ComplexPattern::ColumnMajor => {
                if dimensions.len() >= 2 {
                    ...
                    (1..=prefetch_count).map(|i| current_idx + stride * i).collect()
                } else {
            ...
            ComplexPattern::DiagonalMajor => {
                if dimensions.len() >= 2 {
                    ...
                    (1..=prefetch_count).map(|i| current_idx + cols * i + i).collect()
                } else {
                    // Default to sequential if dimensions unknown
                    (1..=prefetch_count).map(|i| current_idx + i).collect()
                }
            ...
            ComplexPattern::Block { block_height, block_width } => {
                if dimensions.len() >= 2 {
                    let cols = dimensions[1];
                    let row = current_idx / cols;
                    let col = current_idx % cols;
                    ...
                    while remaining > 0 && next_row < *block_height {
                        for col_offset in 0..block_width.min(remaining) {
                            let idx =
                                (block_row * block_height + next_row) * cols + block_col * block_width + col_offset;
                            result.push(idx);
                            remaining -= 1;
                        }
                        next_row += 1;
            ...
            ComplexPattern::Stencil { dimensions: dim_count, radius } => {
                if dimensions.len() >= *dim_count {
                    ...
                    for r in -(*radius as isize)..=(*radius as isize) {
                        for c in -(*radius as isize)..=(*radius as isize) {
                            ...
                            // Check bounds
                            if new_row >= 0 && new_row < dimensions[0] as isize &&
            ...
            _ => {
                ...
                // Deduplicate
                result.into_iter().collect::<HashSet<_>>().into_iter().collect()
            }
        }
    }
}
...
mod tests {
    ...
    fn test_row_major_detection() {
        ...
        assert!(patterns.iter().any(|p| matches!(p.pattern_type, ComplexPattern::RowMajor)));
    ...
    fn test_column_major_detection() {
        ...
        assert!(patterns.iter().any(|p| matches!(p.pattern_type, ComplexPattern::ColumnMajor)));
        
        // Check basic pattern - should be strided
        assert!(matches!(recognizer.get_basic_pattern(), AccessPattern::Strided(_)));
    }
    ...
    fn test_zigzag_detection() {
        ...
        assert!(patterns.iter().any(|p| matches!(p.pattern_type, ComplexPattern::Zigzag)));
    }
    ...
    fn test_diagonal_detection() {
        ...
        assert!(patterns.iter().any(|p| matches!(p.pattern_type, ComplexPattern::DiagonalMajor)));
    }
    ...
    fn test_block_detection() {
        ...
        assert!(patterns.iter().any(|p| {
            if let ComplexPattern::Block { block_height, block_width } = p.pattern_type {
    ...
    fn test_pattern_utils() {
        ...
        let prefetches = pattern_utils::get_prefetch_pattern(
            &pattern, &dimensions, current_idx, prefetch_count
        );
